...
public class ContaCorrente implements IConta{
	...
	public void sacar(float valorSacado) {
		if(valorSacado > 0 && this.saldo >= (valorSacado+(valorSacado*TAXA_SACAR_CONTA_CORRENTE)) && this.status) {
	...
	public void transferencia(IConta contaDestino, float valorTransferido) {
		if(contaDestino instanceof ContaPoupanca) {
			...
			this.sacar(valorTransferido+(valorTransferido*TAXA_ADMINISTRACAO_INVESTIMENTO));
			contaDestino.depositar(valorTransferido);
